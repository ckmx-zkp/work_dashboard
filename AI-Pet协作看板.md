...
| 2026-09-09 | xiaozhi-server / 智控台模型有效性 | 契约已变更（小智 docs/12）：有效性列、逐项检测及后端启用限制已实现；66项实测10有效/43无效/13待验证。完整生产库复制已获用户授权，隔离迁移及12项API验收通过，Java121/Python5/Node18测试与前端构建通过。生产发布被自动审批拦截，待用户明确授权两服务重启；尚未上线。详见小智 docs/13。 |
