...
## 2026-09-23 语音故障归档（用户确认）

- 现象：S3 大板可识别唤醒词，唤醒后无后续语音识别／正常播报，回退旧版仍复现。
- 根因：用户确认火山 TTS 和 ASR 过期。此前 PA_EN／R40、MIC2 增益、AEC、触摸及 sdkconfig 均为排查假设，不作为本次故障根因；speaking/listening 短暂切换不等于音频在 10ms 内中断。
- 状态：根因已确认并归档；未提供续期处理详情及恢复后的端到端验收结果，不登记为已恢复。接口契约未变更，本次仅更新文档。
...
2026-09-08 音乐与内容工具：39.107.143.71 已部署独立音乐网关及全局 MCP，覆盖登记的 4 块实体板子的新会话。四工具与公网 Ogg 流已验证；实际固件缺少 self.music.play_url 时仍不能播放，逐板语音/扬声器验收待完成。部署与回滚见 xiaozhi-server/docs/10-音乐与内容MCP部署-20260908.md。
...
- ✅ **2026-09-20 UI/UX 现代化全面重构已部署**：提交 `05a02fb`；全面重塑管理平台视觉体系。引入科技紫靛设计系统（base.css）、沉浸式双栏品牌登录页、全套矢量图标侧边栏与毛玻璃顶栏、设备资产四维指标卡、详情 Hero 看板与结构化人设分组卡片、KPI 运营仪表盘；本地构建通过，已同步部署至 ECS 39.107.143.71:8080 并热重载 Nginx，首页 200 验证通过。
...
| X8 | 火山方舟视觉多模态大模型接入（Seed 2.0 Mini / Seed Character / Seed 2.0 Pro / Seed 2.1 Pro / Seed 2.0 Lite / GLM 5.3 Flash 实测图文验证通过） | 统一方舟端点，密钥入库 | ✅ 已部署 |
| X9 | 模型Token用量统计与看板展示（方案B：流式切片Usage提取 + 异步上报 + Redis日级与历史聚合 + 智控台查看） | voice 与 manager-api 联动 | ✅ 已部署 |
...
| 2026-09-08 | 音乐MCP / 交付约定 | 用户授权以后相关修改自动验证、构建部署39.107.143.71、提交推送并更新看板；已写入源工程与xiaozhi-server的AGENTS.md。纯文档无需重启服务。源工程D:/xiaozhi_music_mcp尚无Git远端，先本地提交，待提供远端后推送。 |
| 2026-09-08 | backend / 小智 / 数字分身云桥 | 契约已变更：六陪伴工具与28策略、候选审批/约定上线，backend 9be6235、小智b14-companion；139后端及43内容测试、独立PG与MCP写入、线上MiniMax和4板原生客户端通过。朋友数字分身隔离账号/设备，20:14云端注册14工具，含记忆与情绪支持；两份用户说明已落盘。个人数据向MiniMax出站已获明确授权，未做物理验收；源工程fb01dce本地提交，无远端。 |

| 2026-09-09 | xiaozhi-server / 智控台模型有效性 | 契约已变更（小智 docs/12）：有效性列、逐项检测及后端启用限制已实现；用户授权完整生产库副本验收及两服务重启后已上线（manager validity-r2 / voice b15-validity）。线上66项最终13有效/42无效/11待验证，无效及待验证启用/默认数量0。隔离迁移及13项API验收、Java121/Python5/Node18与构建通过；线上静态资源摘要、OTA与登记设备WS hello通过。1个智能体仍引用无效视觉配置待补齐，未擅自替换；未做物理音频验收。详见小智 docs/13。 |

| 2026-09-09 | xiaozhi-server / 模型有效性上线 | 用户明确授权发布并短暂重启；修复JSON null快照误判并回归，66项线上状态已落库，禁用规则与OTA/WS握手通过。契约已变更（docs/12），最终报告docs/13。 |
...
| 2026-09-23 | 固件 / 服务端 | 语音故障归档：用户确认火山 TTS 和 ASR 过期；此前硬件、AEC、增益、触摸及 sdkconfig 推测不作为根因。恢复验收结果未提供；契约未变更。 |
| 2026-09-25 | 固件 | S3 大板喇叭：TTS start 到达时即收 Opus，并提前拉高 GPIO46。增量编译后只烧 app 到 COM28，新镜像 ELF SHA 0698b2c8a 已启动。喇叭是否出声未听验。固件改动未提交。契约未变更。 |
| 2026-09-25 | 固件 | 上述改动已提交并推送 `fix/s3-cam-pa-gpio46` `65d3b9b`。COM28 故事阶段输出已打开但仍无声，GPIO46 电压未测。新增 `esp32-s3-v101`（PCA9557 IO1 做 PA_EN），已编译；COM25 下载无串口应答，未烧入。契约未变更。 |
