...
| 业务后端 | 核心 API、Memory MCP、Worker、KB、dossier、E10、E6.1/E2.1/E7.1/E8、KB v3、趣味测验/星盘已上线 | 🟢 `4b2fc62` 已部署 | App `/tests` 已上；E11 仍冻结 |
...
**当前优先级**：X1 真机收口 > App `d1dd70d` 重新部署与真实账号验收（含 E10 生成链路） > App A8/A9/A10 > E11 主动播报架构拍板 > 固件 F1 > 域名/TLS/监控。E11 未统一前不得并行实现 B10/X4/X5/A15/D6/F7。
...
- ECS 运行实现提交 `5e7e851`，迁移至 `0009_kb_v3_first_person`；本地 `main` 在其后包含部署记录和新增功能开发，最终提交号以本节后续更新为准。
...
- E10/E10.3 两步联网检索仍有效。质量闸 ruff/mypy/pytest 108 全绿。
- E11 主动播报仍因跨仓传输冲突冻结。

**文档地图**：接口真源 `docs/06`；数据模型 `docs/02`；E10 设计 `docs/12`；部署记录 `docs/09`；排期 `docs/10`。

**下一步**：App 接 A8/A9/A10；E10 真实账号验收。E11 先拍板再实现 B10。
...
- 本轮已实现 A1（dossier 六字段全可编辑）、A8 导出 JSON、A9 记忆画像、A10 问卷入口，并修人设保存冲掉 overrides；提交 `ffd04ae`（类型/路由部分已在并行提交 `07f44e5`）。
- A2/A7/A13/A14 代码此前已在 `d1dd70d`，公网曾加载旧资产 `index-Da5xQLPI.js`，将随本轮一并重新部署。
...
详细表见 `AI-Pet协作看板.md`“集成点状态”。主链剩余阻塞：X1 真机证据、App 新构建未在线生效、A14 隐私/HTTPS、固件 F1、正式发布域名/监控；A8/A9/A10 后端前置已解除。E11 另有“在线 WS 轮询 vs 空闲设备 MQTTS 唤醒”跨仓架构冲突。
