...
|---|---|---|---|
| 用户端 | 注册登录、设备绑定码认领已上线 | 🟡 可做内测 | 设备列表、人设和历史页面待接入；正式发布还需域名与 HTTPS |
| 业务后端 | 账号、设备身份、人设、对话历史已上线 | 🟡 主链路基础已具备 | 记忆、分析与运营能力尚未完成；需继续打通语音侧数据回传 |
| 管理台 | 登录、当前账号设备查看与设备基础管理已上线 | 🟡 运营能力建设中 | 管理端资产/诊断接口未提供，暂不能管理真实用户设备 |
| 设备与语音 | 真机激活、首轮语音对话、单眼情绪联动已验证 | 🟡 集成推进中 | 语音服务仍需按新会话标识回传数据；第二只眼及更多外设待完成 |

**本周优先级**：先修复语音服务与后端的数据回传兼容，完成设备首见登记和人设下发；再推进用户端设备列表、人设与历史体验。当前全套线上容器正在运行；展示页、管理台、用户端均已部署，但用户端仍仅适合 HTTP 内测。
...
- ✅ **已实现并上线**：auth（register/login/me，JWT+argon2id+审计）；devices 五端点（绑定/列表/详情/改名/解绑+重绑）；CORS 中间件；persona_compiler 编译纯函数；worker SKIP LOCKED 消费循环（但**任务注册表为空，无处理器**）。
- ⬜ **501 骨架**（25 处，签名/鉴权就位）：admin KB 7 端点、analyses 2、internal 4（persona_pack/chat events/peripheral events/session end）、memories 6、messages 2、peripheral 1、persona 3。
...
**下一步**：打通语音服务的 persona_pack、设备首见登记、转写与会话结束回传；再完成 memories/MCP、分析和运营接口。
...
- ✅ **B1.1 已上线**：已移除管理台调用用户绑定接口的入口，设备认领由用户端使用绑定码完成；当前可查看当前账号设备。
...
**下一步**：等待后端提供独立的管理端设备资产/诊断接口；随后推进人设、历史/记忆、分析与 KB 运营页面。

## 六、ai-pet-app（用户端）

**定位**：手机 PWA + 桌面，用户自服务（登录/绑设备/人设/记忆/日运）。Vue3 应用骨架、账号流程与设备绑定码认领已完成并部署到 ECS `:8081`，GitHub `ckmx-zkp/ai-pet-app-`。

**已完成**：Vue3 + Vite + TS strict 工程、响应式导航、注册/登录与会话保持、`binding_id` 设备认领；类型检查与生产构建通过，已部署内测。
...
**下一步**：设备列表/详情与多设备切换 → 人设设置和首页摘要 → 对话历史。记忆、分析、外设和数据导出仍受后端能力阻塞；正式发布需域名与 HTTPS。
