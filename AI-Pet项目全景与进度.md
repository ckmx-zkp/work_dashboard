...
人格陪伴六工具及12星座/16MBTI策略已部署；backend 9be6235三服务同镜像，139测试与独立PG/MCP写入验收通过；小智b14-companion和内部10工具MCP已验证4块实体板的原生客户端身份/重连。真实MiniMax检索、来源与复用通过，未做固件烧录或扬声器物理验收。

朋友的赛博分身使用独立普通用户/虚拟设备，不接触宠物资料。独立云桥已于20:14向用户指定小智云接入点注册14工具（含长期记忆和情绪支持）。两份同内容说明已写入D:/ESP32-s3-Touch-LCD-1.85C-BOX与D:/xiaozhi_music_mcp。契约与部署记录见backend docs/06、09及小智docs/05、08、11，密钥仅在服务器私有配置。
...
- backend `68d30a5` 已部署为三服务共用镜像：API / Worker / Memory MCP 提交与镜像 ID 一致，API 与 MCP 只读协议检查通过。27 类关系与宠物口吻已在线；本地 ruff/mypy/pytest 130 通过。
- App `9411a7f` 已部署：我的星仔页消费结构化关系目录并独立保存，保留 dossier 六字段；路由按需加载，生成 release.json。生产构建、模拟 API 手机/桌面交互与线上 39 文件 SHA256 验证通过，真实关系采用仍待真机验证。
- Admin `c6d4ae4` 当前构建 4 文件与线上相同；小智 b13 的 180 个受控 Python 文件与本地相同。
- 总仓已快进到远端 28d9fba；SSH 入口按当前 config 为 aliyun-aipet。未跟踪硬件、bundle、logs 保留。
- 下一轮：X1/多板真机验收、人设采用状态契约、Worker 事务/退避、记忆召回评估、Admin 分包与 Ops；E11/BLE 继续待共同契约。下方 2026-08-19 快照中与本段冲突的部署状态，以本段为准。
